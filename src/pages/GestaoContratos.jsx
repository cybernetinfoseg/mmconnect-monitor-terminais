...
import {
  FileText, Plus, Search, Pencil, Trash2, Loader2,
...
const ESTADO_CONFIG = {
  ativo: { label: 'Ativo', cls: 'bg-emerald-100 text-emerald-700 border-emerald-200', icon: CheckCircle2 },
  expirado: { label: 'Expirado', cls: 'bg-red-100 text-red-700 border-red-200', icon: XCircle },
  rescindido: { label: 'Rescindido', cls: 'bg-slate-100 text-slate-600 border-slate-200', icon: XCircle },
  pendente_assinatura: { label: 'Pendente Assinatura', cls: 'bg-amber-100 text-amber-700 border-amber-200', icon: Clock },
};
...
export default function GestaoContratos() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setDialogOpen(false); setEditingId(null); setFormData({});
  ...
  const handleNew = () => {
    setEditingId(null);
    setFormData({ tipo: 'sem_termo', estado: 'ativo', horas_semanais: 40, subsidio_alimentacao: 0, subsidio_alimentacao_tipo: 'cartao_refeicao', aviso_expiracao_dias: 30, renovacao_automatica: false });
    setDialogOpen(true);
  };

  const set = (f, v) => setFormData(prev => ({ ...prev, [f]: v }));

  const diasParaExpirar = (contrato) => {
    if (!contrato.data_fim) return null;
    return differenceInDays(parseISO(contrato.data_fim), hoje);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 w-full">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'Ativos', value: stats.ativos, color: 'text-emerald-600', bg: 'bg-emerald-50' },
            { label: 'A Expirar (30d)', value: stats.aExpirar, color: 'text-amber-600', bg: 'bg-amber-50' },
            { label: 'Expirados', value: stats.expirados, color: 'text-red-600', bg: 'bg-red-50' },
            { label: 'Pend. Assinatura', value: stats.pendentes, color: 'text-blue-600', bg: 'bg-blue-50' },
          ].map((s, i) => (
        ...
        {isLoading ? (
          ...
          <Card className="bg-white">
            <CardContent className="py-16 text-center text-slate-400">
              ...
              <Button onClick={handleNew} className="mt-4 bg-purple-600 hover:bg-purple-700 text-sm">
                <Plus className="h-4 w-4 mr-2" /> Criar contrato
              </Button>
            </CardContent>
          </Card>
        ) : (
                    ...
                    <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <p className="text-xs text-slate-600">
                          ...
                          {c.data_fim ? ` → ${format(parseISO(c.data_fim), 'dd/MM/yyyy')}` : ' → Indeterminado'}
                        </p>
                        {alertaExpiracao && (
                          <Badge className="text-[10px] bg-amber-100 text-amber-700 border-amber-200 mt-0.5">
                      ...
                      <td className="px-4 py-3 text-slate-700 text-sm font-medium">
                        {c.salario_base ? `€${c.salario_base.toLocaleString('pt-PT', { minimumFractionDigits: 2 })}` : '—'}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex justify-end gap-1">
                          <Button size="sm" variant="outline" className="h-7 w-7 p-0" onClick={() => { setEditingId(c.id); setFormData({ ...c }); setDialogOpen(true); }}>
        ...
        <DialogContent className="w-[95vw] max-w-xl max-h-[92vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Fim Período Experimental">
                <Input type="date" value={formData.data_fim_periodo_experimental || ''} onChange={e => set('data_fim_periodo_experimental', e.target.value)} />
              </Field>
              <Field label="Horas Semanais">
                <Input type="number" value={formData.horas_semanais ?? 40} onChange={e => set('horas_semanais', Number(e.target.value))} />
              </Field>
              <Field label="Salário Base (€)">
                <Input type="number" step="0.01" value={formData.salario_base || ''} onChange={e => set('salario_base', parseFloat(e.target.value))} placeholder="0.00" />
              </Field>
              <Field label="Subsídio Alimentação/dia (€)">
                <Input type="number" step="0.01" value={formData.subsidio_alimentacao ?? 0} onChange={e => set('subsidio_alimentacao', parseFloat(e.target.value))} />
              </Field>
              ...
              <Field label="Aviso Expiração (dias)">
                <Input type="number" value={formData.aviso_expiracao_dias ?? 30} onChange={e => set('aviso_expiracao_dias', Number(e.target.value))} />
              </Field>
            </div>
          ...
          <div className="flex gap-2 pt-3 border-t border-slate-100 mt-2">
            ...
            <Button className="flex-1 bg-purple-600 hover:bg-purple-700" disabled={saveMutation.isPending || !formData.colaborador_id || !formData.tipo || !formData.data_inicio} onClick={() => saveMutation.mutate(formData)}>
              ...
              {editingId ? 'Guardar' : 'Criar Contrato'}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
