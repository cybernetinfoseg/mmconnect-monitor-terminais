              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col overflow-hidden shadow-sm min-h-[25vh]">
                ...
                <div className="space-y-1.5 flex-1 overflow-y-auto pr-1">
                  {opLogs.length === 0 ? (
                    ...
                    opLogs.map(log => (
                      <div key={log.id} className="flex items-start gap-2 p-2 rounded-lg bg-slate-50 border border-slate-100 transition-colors hover:bg-slate-100/50">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-1.5 flex-wrap">
                            <Badge className="text-[9px] bg-slate-200 text-slate-600 px-1.5 border-none">{log.acao}</Badge>
                          ...
                          <p className="text-[11px] text-slate-700 mt-1 break-words">{log.mensagem || '—'}</p>
                        </div>
                      </div>
                    ))
                  )}
