...
import { TrendingUp, Plus, Search, CheckCircle2, Loader2, AlertTriangle, Archive, TrendingDown, Clock, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function TabHorasExtra({ currentUser, colaboradores }) {
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      ...
      return base44.entities.RegistoHorasExtra.create({
        ...data, fator_aplicado: fator,
        minutos_compensados: Math.round((data.minutos_extra || 0) * fator),
        acumulado_ano_antes: acumulado, aprovado: false, processado: false,
        destino: data.destino || 'pendente', owner_email: currentUser?.email,
      });
    },
    onSuccess: () => { queryClient.invalidateQueries(['horas-extra']); setDialogOpen(false); setFormData({}); toast.success('Registo criado'); },
  ...
  const aprovarMutation = useMutation({
    mutationFn: async (id) => {
      ...
      if (!registo) return;
      await base44.entities.RegistoHorasExtra.update(id, { aprovado: true, aprovado_por: currentUser?.email, aprovado_em: new Date().toISOString() });
      if (registo.destino === 'banco_horas') {
        ...
        if (saldos.length > 0) {
          await base44.entities.BancoHoras.update(saldos[0].id, { minutos_credito: (saldos[0].minutos_credito || 0) + (registo.minutos_compensados || registo.minutos_extra) });
        } else {
          await base44.entities.BancoHoras.create({ colaborador_id: registo.colaborador_id, colaborador_nome: registo.colaborador_nome, enrollid: registo.enrollid, ano, minutos_credito: registo.minutos_compensados || registo.minutos_extra, minutos_debito: 0, minutos_pagos: 0, owner_email: currentUser?.email });
        }
      }
    },
    onSuccess: () => { queryClient.invalidateQueries(['horas-extra']); queryClient.invalidateQueries(['banco-horas']); setAprovacaoId(null); toast.success('Aprovado'); },
    onError: e => toast.error(`Erro: ${e.message}`),
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <Button size="sm" onClick={() => { setFormData({ tipo_dia: 'util', destino: 'banco_horas', data: new Date().toLocaleDateString('en-CA') }); setDialogOpen(true); }} className="bg-violet-600 hover:bg-violet-700 gap-1.5 self-start">
        ...
        return (
          <div key={colId} className={cn('flex items-center gap-3 px-4 py-3 rounded-xl border text-sm', min >= LIMITE_ANUAL_MIN ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200')}>
            ...
            <span className={min >= LIMITE_ANUAL_MIN ? 'text-red-700' : 'text-amber-700'}>
              <strong>{col?.nome || colId}</strong>: {fmtMin(min)} ({pct}%){min >= LIMITE_ANUAL_MIN && ' — LIMITE ATINGIDO'}
            </span>
          </div>
        );
      })}
              ...
              {filtered.map(r => {
                ...
                return (
                  <tr key={r.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-center"><Badge variant="outline" className="text-xs">{r.tipo_dia === 'util' ? 'Útil' : r.tipo_dia === 'descanso' ? 'Descanso' : 'Feriado'}</Badge></td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <Badge className={cn('text-xs', r.destino === 'banco_horas' ? 'bg-blue-100 text-blue-700' : r.destino === 'pagamento' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500')}>
                        {r.destino === 'banco_horas' ? 'Banco' : r.destino === 'pagamento' ? 'Pagamento' : 'Pendente'}
                      </Badge>
                    </td>
                    <td className="px-4 py-3 text-center">
                      {r.aprovado ? <Badge className="text-xs bg-emerald-100 text-emerald-700">Aprovado</Badge> : <Badge className="text-xs bg-amber-100 text-amber-700">Pendente</Badge>}
                    </td>
        ...
        <DialogContent className="w-[95vw] max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5"><Label className="text-xs font-medium text-slate-600">Data *</Label><Input type="date" value={formData.data || ''} onChange={e => set('data', e.target.value)} /></div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5"><Label className="text-xs font-medium text-slate-600">Horas</Label><Input type="number" min="0" max="24" placeholder="0" value={formData._horas || ''} onChange={e => { set('_horas', e.target.value); set('minutos_extra', hm(e.target.value, formData._minutos)); }} /></div>
              <div className="space-y-1.5"><Label className="text-xs font-medium text-slate-600">Minutos</Label><Input type="number" min="0" max="59" placeholder="0" value={formData._minutos || ''} onChange={e => { set('_minutos', e.target.value); set('minutos_extra', hm(formData._horas, e.target.value)); }} /></div>
            </div>
            ...
            <div className="space-y-1.5"><Label className="text-xs font-medium text-slate-600">Motivo</Label><Textarea value={formData.motivo || ''} onChange={e => set('motivo', e.target.value)} rows={2} /></div>
          </div>
      ...
      <Dialog open={!!aprovacaoId} onOpenChange={open => !open && setAprovacaoId(null)}>
        <DialogContent className="w-[95vw] max-w-sm">
          ...
          {aprovacaoId && (() => { const r = registos.find(x => x.id === aprovacaoId); return r ? (
