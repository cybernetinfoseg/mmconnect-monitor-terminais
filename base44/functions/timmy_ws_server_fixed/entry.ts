...
def obter_hora_sincronizada():
    ...
    try:
        ...
        logger.warning(f"[TIMEZONE] Falha ao renderizar tz '{USER_TIMEZONE}', usando UTC: {e}")
...
def parse_log_record(rec, terminal_id, terminal_nome, terminal_local):
    ...
    if enrollid <= 0 or enrollid >= 99999999:
        logger.warning(f"[FILTRO] Tentativa invalida ignorada no terminal '{terminal_nome}' (enrollid={enrollid}).")
        return None
...
class CtrlHandler(BaseHTTPRequestHandler):
    ...
    async def _send_and_wait(self, ws, sn, command):
        ...
        try:
            ...
            logger.info(f"[HTTP-API] Comando enviado para SN={sn}")
...
def start_ctrl_server(port, stop_event):
    try:
        ...
        server.timeout = 1
        logger.info(f"[CTRL] Servidor HTTP ativo em http://0.0.0.0:{port}")
                ...
                for sn in sns_conectados:
                    try:
                        ...
                        if ws:
                            ...
                            nome = sn_to_nome.get(sn, sn)
                            logger.info(f"[RELOAD-SYNC] settime enviado para '{nome}' (SN={sn}) tz={USER_TIMEZONE}")
                    except Exception as e:
                        logger.error(f"[RELOAD-SYNC] Erro SN={sn}: {e}")

            logger.info(f"[RELOAD] {len(sn_to_terminal)} terminais | tz={USER_TIMEZONE}")
        except Exception as e:
...
async def main_async(ws_port, stop_event):
    ...
    async with serve(handle_terminal, "0.0.0.0", ws_port, ping_interval=None) as server:
        logger.info("="*60)
        logger.info("Timmy WebSocket Server - NOC Monitor (Dinamico)")
        logger.info(f"WS Port: {ws_port}")
        logger.info(f"CTRL Port: {_config.get('CTRL_PORT', DEFAULT_CTRL_PORT)}")
        logger.info(f"Terminais: {len(sn_to_terminal)}")
